...
def test_exotic_tags(xmloutput=False):
    ...
    result = load_mock_page('http://exotic_tags',
                            xml_flag=xmloutput, tei_output=True)
    ...
    result = etree.tostring(handle_lists(etree.fromstring(
        '<list>Description of the list:<item>List item 1</item><item>List item 2</item><item>List item 3</item></list>'), False, ZERO_CONFIG))
...
def test_formatting():
    ...
    my_result = extract(my_document, output_format='xml',
                        include_formatting=True, config=ZERO_CONFIG)
...
def test_filters():
    ...
    if LANGID_FLAG is True:
        ...
        # comments
        assert trafilatura.filters.language_filter(
            'Hier ist ein Text.', 'Die Kommentare sind aber etwas länger.', 'de', SAMPLE_META) is False
    else:
        # no detection
        assert trafilatura.filters.language_filter(
            'Hier ist ein Text.', '', 'en', SAMPLE_META) is False
    # test URL blacklist
    assert trafilatura.extract('<html><head><link rel="canonical" href="https://example.org"/></head><body></body></html>',
    ...
    assert extract(html.fromstring('<html lang="en-US"><body>' +
                   my_p*50 + '</body></html>'), target_language='en') is not None
    assert extract(html.fromstring('<html lang="en-US"><body>' +
                   my_p*50 + '</body></html>'), target_language='de') is None
...
def test_external():
    ...
    # strip fancy tags
    mydoc = html.fromstring(
        '<html><body><p>Text here <fancy>Test text</fancy></p></body></html>')
    ...
    if LANGID_FLAG is True:
        ...
        assert extract(doc, no_fallback=False,
                       target_language='en', deduplicate=False) is None
...
def test_images():
    ...
    assert 'test.jpg Example image' in extract(
...
def test_links():
    ...
    assert 'testlink.html' in extract(
        mydoc, include_links=True, no_fallback=True, config=ZERO_CONFIG)
    resources_dir = os.path.join(TEST_DIR, 'resources')
    with open(os.path.join(resources_dir, 'http_sample.html')) as f:
        teststring = f.read()
    assert 'testlink.html' not in extract(teststring, config=ZERO_CONFIG)
    assert '[link](testlink.html)' in extract(
        teststring, include_links=True, no_fallback=True, config=ZERO_CONFIG)
    assert '<ref target="testlink.html">link</ref>' in extract(
        teststring, include_links=True, no_fallback=True, output_format='xml', config=ZERO_CONFIG)
